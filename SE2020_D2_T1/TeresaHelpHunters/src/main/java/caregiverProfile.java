...
public class caregiverProfile extends HttpServlet {
	...
	String description = req.getParameter("description");
		...
		Vector<reviewData> caregiverReviews;
        caregiverReviews = reviewData.getCaregiverReviews(connection, caregiverID);
         for(int i=0; i< caregiverReviews.size(); i++){
                reviewData review = caregiverReviews.elementAt(i);
				toClient.println("<li>");
		...
		toClient.println("</div>");
		toClient.println("<form name = 'messages' id = 'sendMessage' method='post' action = 'sendMessage'>");
        ...
        toClient.close();
		int n = caregiverData.viewCProfile(connection, caregiverID);
